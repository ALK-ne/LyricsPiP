...
> 詳細な経緯は上の番号付き項目を参照。ここは現時点のサマリのみ。

**動作している主機能(いずれも実機確認済み)**

- 中核機能(Spotify曲検知→歌詞取得→PIP同期表示)がエンドツーエンドで動作。プレイリスト・広告挟み込みでも継続。
- PiPは完全自動運用(バックグラウンドで自動開始・フォアグラウンドで自動終了、手動ボタンなし)。表示行数はユーザー設定で変更可(次1〜5行/1行前表示=②③、項目27)。
- 横画面はアプリ内の全画面歌詞表示(①横画面PiP拡大はiOS制約で断念した代替、項目28)。タップ回転ボタン(回転ロックONでも可)・曲タイトル/アーティスト表示・連続スクロール+上下フェードまで実装(項目28〜38、最新 v1.3.7)。②③は横画面にも反映(②=密度/③=現在行位置)。
- アプリUIはiOS標準デザインに整備(項目31、v1.3.0)。アプリアイコンも新規デザイン済み。

**運用・基盤**

- バージョンは1.0基準のセマンティックバージョニング風。**現在 1.3.7**。
- WebSocket切断は専用URLSessionConfigurationで大幅改善(PiP常時自動起動で実質ほぼ発生せず)。
- 画面上のデバッグログパネルは非表示化済み(リモートログ送信は継続動作)。
- リポジトリはパブリック化済み。GitHub課金($8.74)の免除依頼は返答待ち。

**既知の制約・未着手**

- lrclib/Spotifyどちらにも同期歌詞が無い曲は「歌詞なし」表示(仕様上の制約)。
- YouTube Music / Amazon Music対応は実現可能性調査のみ、未着手(項目24)。
