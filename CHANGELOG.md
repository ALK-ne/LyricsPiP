...
- `CFBundleShortVersionString` を 1.2.2 → **1.2.3**(見た目の調整、パッチ)。
- 横画面の全画面歌詞ビュー(LandscapeLyricsView)とPiPは表示専用面なので今回は変更せず維持。

### 現状・残タスク

- アプリUIをiOS標準デザインに整えた(項目31、v1.2.3)。
...
- バージョン表記は1.0基準のセマンティックバージョニング風に統一済み(項目26)。現在は **1.2.3**(アプリUIをiOS標準デザインに整備)。
