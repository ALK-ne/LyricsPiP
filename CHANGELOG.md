...
- **中核機能(Spotify曲検知→歌詞取得→PIP同期表示)は実機でエンドツーエンド動作確認済み**。プレイリスト再生・広告挟み込みでも動作継続を確認。
- リポジトリはパブリックに切り替え済み。GitHub課金($8.74)の免除依頼は返答待ち。
- 残タスク: WebSocketが約60秒ごとに`Software caused connection abort`で切れる件(iOS固有、再接続で自動回復するが未解明)の安定化。lrclibに同期歌詞が無い曲は「歌詞なし」表示になる(仕様上の制約)。
