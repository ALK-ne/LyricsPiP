...
- 表示行数の設定(v1.1): 画面右上の歯車アイコンから、PIPに表示する「次の行数」を1〜5行で選べる(デフォルト1)。行数を増やすとPIPウィンドウが縦に大きくなる。「1行前も表示」をオンにすると現在行が中央に配置され、前の歌詞も見える(デフォルトOFF)。
- 横画面での全画面歌詞表示(v1.2): アプリを開いたまま横向きにすると、全画面で歌詞を表示する(内容・設定はPIPと同じものを反映)。アプリ自身の画面なのでiOSのPIPサイズ・位置制約を受けず大きく表示できる。縦に戻すと通常画面に戻る。
- 横画面の曲情報ヘッダー(v1.2.2): 横画面の全画面歌詞でも、上部に曲タイトルとアーティスト名を表示する。
- 横画面切り替えボタン(v1.2.1): 縦画面の横向きアイコンをタップすると全画面歌詞(横画面)へ、横画面の縦向きアイコンで元に戻る。iOSの画面回転ロックがONでもボタンなら切り替えられる。強制した向きは、端末を物理的に逆向きへ倒すと自動で追従して戻る。
