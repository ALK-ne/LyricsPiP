...
- **安全装置(`spotify_poll_blocked_until`の`UserDefaults`永続化)が無いビルドでは24時間ブロックが記録されません**。開発初期に一度この状態で24時間ブロックを受けたことがあり、その時点のビルドはまだ永続化機能が無かったためブロック情報がメモリ上にしか残らず、アプリの再起動(≒新しいビルドへの置き換え)で消え、次の起動時に本物のポーリングが自動再開してサーバー側の制限に再度引っかかる、ということが実際に起きました。永続化機能を実装したビルドが実機に入って以降は、Sideloadlyでの再インストール(同一Bundle IDへの上書き)を挟んでも`UserDefaults`の内容は保持され、安全装置は正しく機能し続けています。
