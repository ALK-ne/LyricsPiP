...
サイドロードには[Sideloadly](https://sideloadly.io)を使用しています。
...
**既知の落とし穴**: 「Local Anisette」関連の致命的エラー(`iTunesCore.dll`が見つからない/壊れている)や、ログイン時の404エラーが発生することがある。この場合はSideloadlyを**完全にアンインストールしてから再インストール**し、`%LOCALAPPDATA%\Sideloadly`のキャッシュ状態を消してから再構成すると解消する。エラーが出てもAltServerへ戻さず、Sideloadly自体を直す方針で対応する。
...
- 無料Apple IDの証明書は7日で失効します。**Sideloadlyには証明書を自動更新し続けるバックグラウンド常駐機能はない**ため、7日ごとに手順3を再実行してインストールし直す必要があります。
...
- 曲検知(sp_dc認証 → currently-playingポーリング): 実装済み。実際のSpotify再生を使った通し確認はレート制限の解除待ち。
- 歌詞取得・同期(lrclib.net → LRCParser → LyricsSyncEngine): 実装済み。lrclib.net単体では動作確認済み(後述)。
- PIP表示: 実機で動作確認済み。ダミー歌詞での表示・長い行の自動縮小・他アプリ/ホーム画面切り替え後のバックグラウンド維持、いずれも確認済み。
...
- **`currently-playing`ポーリングには常時レート制限がかかっています**。実測(`tools/spotify-auth-repro.mjs`)では約30〜35秒に1回という恒常的な制限で、アカウント単位ではなくIPアドレス/クライアント種別単位と見られます(別アカウントでも同様に制限される)。`PlaybackPoller`は40秒間隔でポーリングし、制限中はアプリ内にオレンジの警告バナーを表示、解除時刻を`UserDefaults`に永続化して再起動をまたいでも守ります。
