...
- **このレート制限はSpotifyアカウント単位ではありません**。別アカウントの`sp_dc`クッキーで同じPCから`tools/spotify-auth-repro.mjs`を実行しても同様に429(`Retry-After`は数十秒)が返ることを確認済みです。IPアドレス/ネットワーク単位、またはTOTPベースのクライアント種別単位でかかっていると考えられ、アカウントを切り替えても回避できません。
- **24時間ブロック中の再インストールは危険です**。`PlaybackPoller`の安全装置(`spotify_poll_blocked_until`)は`UserDefaults`に保存されており、アプリを再インストールすると消えます。一方`sp_dc`クッキーは**Keychainに保存され、再インストールしても残ります**。つまり24時間ブロック中にSideloadlyで再インストールすると、ログイン状態は保持されたまま安全装置だけが消え、起動時に本物のポーリングが自動再開してしまい、サーバー側の制限がまだ生きていた場合は再度86400秒ペナルティを受け直す可能性があります(実際にこの日、10:25の初回ブロックが12:19頃に再発生したことをログから確認済み)。ブロック中はダミー歌詞での動作確認に留め、再インストール後にログイン状態のまま放置しないよう注意してください。
...
- **PIPのバックグラウンド維持は実機で確認済みです**。ダミー歌詞の状態で、Spotifyアプリやホーム画面に切り替えた後もPIPウィンドウが表示され続けることを確認しました(`silence.m4a`の無音ループ再生によるオーディオセッション維持が機能しています)。
